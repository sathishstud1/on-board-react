...
import createJson from "./Recreate";
...
class PageLayout extends React.Component {
  ...
  addElements = (lines, refVal) => {
    ...
    Object.assign(
      newJsonValues,
      this.state.jsonValues,
      processFields.defaultValues
    );
    ...
    let newReqFileds = [
      ...this.addedReqFields[this.state.currentPageId],
      ...processFields.reqFields,
    ];
  ...
  removeElement = (lines, refVal, removeId) => {
    ...
    let processFields = ProcessFields.removeFields(
      lines,
      removeId,
      this.addedReqFields[this.state.currentPageId],
      this.addedFields,
  ...
  onChangeHandler = function (e) {
    ...
    if (e.target.type == "radio") {
      ...
      this.state.jsonValues[e.target.id + "_selectedLabel"] =
  ...
  saveform = () => {
    let customeOnboardNewJson = createJson.create(
      this.state.jsonValues,
      this.state.recreateArray,
      this.recreateLines,
      this.state.customerOnboardJson
    );
    let validateFields = [];
    ...
    let isValid = Validation.validateForm(
      validateFields,
      this.state.jsonValues
    );
    if (!isValid) {
    ...
    axios
      .post(URL, customeOnboardNewJson)
      .then((response) => {
        this.Alert(
          true,
  ...
  validatePage = () => {
    ...
    if (this.addedReqFields[this.state.currentPageId]) {
      validateFields = [
        ...this.reqFields[this.state.currentPageId],
        ...this.addedReqFields[this.state.currentPageId],
      ];
    } else {
  ...
  renderPage = (Page, PageId, PageLength) => {
    ...
    return (
      <div ref={refId} key={"createPage" + PageId} style={divstyle}>
        <CreatePage
          Page={Page}
  ...
  render() {
    ...
    btns.push(
      <button
        ...
        onClick={() => {
          if (this.validatePage()) {
            const pageId = PageNavigation.previousPage(
    ...
    btns.push(
      <button
        ...
        onClick={() => {
          if (this.validatePage()) {
            const pageId = PageNavigation.nextPage(
              this.state.currentPageId,
              this.PageLength,
              "ShowPage",
              "previousBtn",
    ...
    return (
      <div>
        <div className="border-bottom">{tabs}</div>
        <div>
          <form class="needs-validation" novalidate>
            {items}{" "}
          </form>
        </div>
        <div className="text-right float-right">{btns}</div>
